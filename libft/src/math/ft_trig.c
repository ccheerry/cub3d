...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/07 12:00:00 by acerezo-          #+#    #+#             */
/*   Updated: 2025/09/07 12:00:00 by acerezo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lft.h"

static const float	g_sin_table[361] = {
...
float	ft_normalize_deg(float degrees)
{
	...
	if (degrees >= 0.0f && degrees < 360.0f)
		return (degrees);
	int_deg = *(t_u32*)&degrees;
	if (degrees < 0.0f)
	{
		degrees = -degrees;
		int_deg = *(t_u32*)&degrees;
		normalized = int_deg % 360;
...
{
	static const float	deg_to_rad_factor = 0.017453292f;
...
float	ft_sin_deg(float degrees)
{
	...
	degrees = ft_normalize_deg(degrees);
	deg_int = (int)degrees;
	frac = degrees - deg_int;
	result = g_sin_table[deg_int];
	if (frac > 0.0f && deg_int < 360)
		result += frac * (g_sin_table[deg_int + 1] - g_sin_table[deg_int]);
	return (result);
}

float	ft_cos_deg(float degrees)
{
	return (ft_sin_deg(degrees + 90.0f));
}